...
const approvedProfileSource = {
  identity: {
    ...
    headline: fact(
      'identity-headline',
      '안녕하세요. 데이터 파이프라인과 AI 서비스를 만드는 조준희입니다.',
    ),
  },
  narrative: {
    shortIntro: fact(
      'narrative-short-intro',
      '데이터를 수집하고 가공해 검색과 생성형 AI 서비스로 연결하는 파이프라인을 만들고 운영합니다.',
    ),
    detailedIntro: [
      paragraph(
        'narrative-detailed-intro-0-text',
        '의료기술 제조사 소프트웨어 인턴십으로 개발 경험을 시작했고, AI/ML 논문 코퍼스 팀 프로젝트에서 데이터 엔지니어로 수집·색인 파이프라인과 인프라 운영을 맡았습니다. 데이터 정제, 장애 복구, 의미 기반 검색, LLM 연동을 경험했고, 지금은 데이터 엔지니어링과 AI 서비스 개발을 중심으로 작업을 쌓고 있습니다.',
      ),
    ],
    resumeSummary: fact(
      'narrative-resume-summary',
      'AI/ML 논문 코퍼스의 수집·색인 파이프라인을 구축하고 운영했으며, 의료기술 제조사 인턴십에서 소프트웨어 개발을 경험했습니다. 데이터 정제부터 검색·요약 서비스 연결까지 담당했습니다.',
    ),
    portfolioSummary: fact(
      'narrative-portfolio-summary',
      '수집·가공·색인부터 검색과 LLM 연결까지, 데이터가 제품에서 쓰이는 전 구간을 설계하고 운영합니다. DocSuri에서 논문 코퍼스 파이프라인과 장애 복구를 맡은 기록, 개인 프로젝트에서 의미 기반 검색과 LLM 연동을 만든 경험을 소개합니다.',
    ),
  },
  contact: {
  ...
  skillGroups: [
    {
      ...
      skills: [
        ...
        { id: 'rust', order: 20, name: fact('skill-rust-name', 'Rust') },
        {
          id: 'typescript',
          order: 30,
        ...
        { id: 'cpp', order: 50, name: fact('skill-cpp-name', 'C++') },
        { id: 'c', order: 60, name: fact('skill-c-name', 'C') },
        { id: 'dart', order: 70, name: fact('skill-dart-name', 'Dart') },
      ],
    },
    {
      id: 'backend-data',
      order: 20,
      title: fact('skill-group-backend-data-title', '데이터·백엔드'),
      skills: [
        { id: 'sql', order: 10, name: fact('skill-sql-name', 'SQL') },
        {
          id: 'postgresql',
          order: 20,
          name: fact('skill-postgresql-name', 'PostgreSQL'),
        },
        {
          id: 'apache-kafka',
          order: 30,
        ...
        { id: 'redis', order: 40, name: fact('skill-redis-name', 'Redis') },
        {
          id: 'opensearch',
          order: 50,
          name: fact('skill-opensearch-name', 'OpenSearch'),
        },
        {
          id: 'mysql-mariadb',
          order: 60,
    ...
    },
    {
      id: 'ai-llm',
      order: 25,
      title: fact('skill-group-ai-llm-title', 'AI·검색'),
      skills: [
        {
          id: 'amazon-bedrock',
          order: 10,
          name: fact('skill-amazon-bedrock-name', 'Amazon Bedrock'),
        },
        {
        ...
        },
        {
          id: 'chromadb',
          order: 30,
          name: fact('skill-chromadb-name', 'ChromaDB'),
        },
        { id: 'ollama', order: 40, name: fact('skill-ollama-name', 'Ollama') },
      ],
    },
    {
      id: 'infrastructure',
      order: 30,
      title: fact('skill-group-infrastructure-title', '인프라·배포'),
      skills: [
        {
          id: 'docker',
          order: 10,
        ...
        { id: 'aws', order: 30, name: fact('skill-aws-name', 'AWS') },
        { id: 'nginx', order: 40, name: fact('skill-nginx-name', 'NGINX') },
        {
          id: 'github-actions',
          order: 50,
  ...
  projects: [
    {
      ...
      outcomeSummary: fact(
        ...
        'AI/ML 논문을 수집·구조화·임베딩·색인하는 파이프라인을 만들고 운영했습니다. 대량 백필로 발생한 검색 503 장애를 복구하고, 사용자 열람용 문서를 생성하는 큐와 워커를 분리했습니다.',
      ),
      problem: [
        paragraph(
          'project-docsuri-problem-0-text',
          'DocSuri는 AI/ML 논문을 탐색하고 원문에 근거한 한국어 요약을 제공하는 서비스입니다. 제가 맡은 수집 데이터는 검색, 문서 열람, 요약에 사용됩니다. 출처가 다른 논문의 중복과 버전, 철회를 처리하면서, 과거 데이터를 대량으로 채우는 백필이 사용자 응답을 방해하지 않도록 해야 했습니다.',
        ),
        paragraph(
          'project-docsuri-problem-backfill-contention',
          '백필이 검색에 미친 영향\n2026년 7월 AWS 운영 중 대량의 arXiv 백필이 OpenSearch 조회를 늦추면서 k-NN 검색이 2초 제한을 넘겨 타임아웃 503 오류를 반환했습니다. 사용자 열람용 문서 생성 작업도 약 3.3만 건의 백필 뒤에서 대기했습니다. 각 프로세스가 요청 한도를 지켜도 워커를 3개 실행하면 전체 요청량은 늘어난다는 점을 확인했습니다.',
        ),
      ],
      role: [
        paragraph(
          'project-docsuri-role-0-text',
          'AWS AI School 2기 4인 팀의 데이터 엔지니어로서 논문 코퍼스의 수집·구조화·임베딩·색인 파이프라인과 인프라 운영, 후속 개선을 맡았습니다. 검색·요약·에이전트의 초기 구현과 GROBID TEI 구조 파서는 다른 팀원이 담당했습니다.',
        ),
        paragraph(
          'project-docsuri-role-contract-collaboration',
          '데이터를 주고받는 규칙과 협업\n수집 데이터의 필드나 의미를 바꿀 때 공용 JSON Schema와 Python DTO뿐 아니라 그 데이터를 쓰는 요약 기능과 프런트엔드도 함께 수정했습니다. 호출 동작은 Protocol 인터페이스로 분리하고, 데이터를 만드는 쪽과 쓰는 쪽의 작업·검토 책임을 정리했습니다. 문서 추적성 도구에는 한국어 요구사항 ID, 모노레포 경로, 유닛 계획을 해석하는 기능을 보완해 검토할 문서 간 연결을 찾을 수 있게 했습니다.',
        ),
      ],
      keyDecisions: [
        paragraph(
          'project-docsuri-key-decisions-0-text',
          '수집량을 제한하고 사용자 작업을 분리\n7월 1일 수집 워커를 3개에서 1개로 줄여 검색을 복구하고, 사용자 열람 문서 생성 작업에 전용 우선순위 큐를 마련했습니다. 7월 2일 별도 워커 서비스로 옮겨 실행 자원을 분리했습니다. 외부 데이터 소스의 요청 한도에 맞춰 확장 상한을 정하고, bulk 작업은 30분, 열람 문서 생성은 5분 이상 대기하면 알람이 울리게 했습니다. DLQ 재처리 절차에는 검색 지연도 함께 확인하도록 명시했습니다.',
        ),
        paragraph(
          'project-docsuri-decision-canonical-records',
          '여러 출처의 논문을 하나의 레코드로 통합\nDOI, arXiv ID, 정규화한 제목·첫 저자·연도 순으로 식별 키를 선택하고, 출처별 우선순위에 따라 대표 레코드를 정했습니다. 우선순위가 낮은 중복 데이터는 원문 추출과 임베딩 전에 걸렀고, 대표 레코드에는 같은 논문의 다른 식별자와 발견한 출처를 남겼습니다. 버전과 우선순위 조건에 따라 레코드를 갱신하고, 철회된 논문은 청크·캐시·자산과 대표 레코드 상태를 정리하도록 했습니다.',
        ),
        paragraph(
          'project-docsuri-decision-retry-watermarks',
          '재시도와 완료 상태를 구분\n같은 버전 메시지가 다시 전달되는 경우와 bulk 색인의 일부 항목이 실패하는 경우를 테스트했습니다. 실패한 항목은 완료로 기록하지 않고, 재시도 메시지와 DLQ에는 출처와 실패 단계를 남겼습니다. 출처별 처리 지점인 watermark가 뒤로 이동하지 않고 서로 독립적으로 관리되는지는 속성 테스트로 확인했습니다. 인덱스 재구축 중에는 스케줄·이벤트 등록과 기존 작업 실행을 제한했습니다.',
        ),
        paragraph(
          'project-docsuri-decision-reprocessing-modes',
          '변경 목적에 맞춘 재처리\n저장 구조가 바뀌면 기존 레코드를 복사하고, 모델이 바뀌면 저장된 청크를 다시 임베딩하며, 파서가 바뀌면 캐시의 원본 바이트를 다시 파싱하도록 처리 시작점을 나눴습니다. 열람용 문서를 다시 만드는 작업과 검색용 청크·벡터를 갱신하는 작업도 구분해 불필요한 원문 재수집을 줄였습니다.',
        ),
        paragraph(
        ...
        paragraph(
          ...
          '외부 문서를 가져올 때 입력 검증\n접속 대상의 공인 IP와 리다이렉트를 검사하고, 스트리밍 응답 크기, XML 엔티티 확장, 압축 해제에 대한 제한을 보완했습니다. 6월 29일 기록의 크기 상한은 외부 응답 64MB, XML 32MB, 압축 파일의 개별 항목 20MB와 합계 200MB입니다.',
        ),
        paragraph(
      ...
      architecture: [
        paragraph(
          ...
          '논문 수집부터 검색·열람·요약까지\narXiv, Semantic Scholar, OpenAlex에서 가져온 논문을 수집 큐에 넣고, 문서 구조화와 청크·임베딩 생성을 거쳐 OpenSearch에 색인했습니다. AWS 운영 당시에는 Python, PostgreSQL, SQS, ECS/Fargate, S3, Bedrock, CDK, CloudWatch, Redis를 사용했고, 사용자 열람용 문서 생성은 별도 큐와 워커로 처리했습니다.',
        ),
        paragraph(
          'project-docsuri-architecture-docmodel-contract',
          '본문과 검색 청크의 기준이 되는 DocModel\n문서의 섹션과 블록을 공통 구조로 표현하고, 이를 바탕으로 읽기 순서에 맞춘 본문과 검색 청크를 만들었습니다. 문단·표·수식·그림 캡션에는 논문·버전·섹션·블록 식별자를 연결하고, 검색 토큰과 원문 블록을 참조하는 blockRefs를 분리했습니다. 저장 데이터의 호환성은 파서와 스키마 버전으로 판단하며, 파생 캐시에서도 DocModel 세대를 구분합니다.',
        ),
        paragraph(
        ...
        paragraph(
          ...
          '검색 색인과 그림·도표 생성을 분리\n팀의 그림·도표 파이프라인은 주 색인 처리가 끝난 뒤 별도로 실행하며, 성공을 필수 조건으로 삼지는 않습니다. 객체 저장소에 먼저 저장한 뒤 DB의 자산 목록인 manifest를 갱신하고, 문서 블록의 assetId와 연결합니다. 검색 가능 상태와 화면에 표시할 자산이 준비된 상태를 구분하며, 버전이 바뀌거나 논문이 철회되면 파생 자산을 정리합니다.',
        ),
        paragraph(
          'project-docsuri-architecture-user-pdf',
          '사용자가 업로드한 PDF의 처리 규칙\n공개 논문과 구별하도록 userdoc 식별자와 별도 작업 종류를 정의했습니다. 작업 정보, 소유자, 참조 레코드가 서로 일치하는지 검사하고, 저장된 PDF를 공통 DocModel로 변환했습니다. 잘못된 입력이나 파싱할 수 없는 문서는 재시도로 해결되지 않는 영구 실패로 분류해 DLQ로 보냈습니다. 이는 작업 메시지를 검증하는 규칙이며, 읽기 API에서 소유자를 확인하는 문제까지 해결한 것은 아닙니다.',
        ),
        paragraph(
        ...
        paragraph(
          ...
          '배포 설정과 실제 동작을 대조\n자산 조회에 필요한 S3 prefix 범위의 권한을 보완하고, 지표 namespace에 맞는 IAM 권한과 실제 지표 전달 여부를 대조했습니다. 기존 큐를 다시 생성하려다 충돌하는 것을 확인한 뒤 import 방식으로 가져왔으며, 기능 플래그는 보존했습니다. 커밋 태그, 서비스 안정화 상태, health 검사로 배포를 추적했고, 사용이 끝난 도메인의 기본값을 정리할 때도 재현용 테스트 데이터와 식별용 namespace는 남겼습니다.',
        ),
        paragraph(
        ...
        paragraph(
          ...
          '중복 이벤트와 실패 원인을 구분\n검색 이력에서는 requestId로 같은 이벤트가 다시 전달된 경우와 사용자가 같은 질의를 새로 실행한 경우를 구분합니다. 임베딩 장애에 대응해 어휘 기반 검색으로 대체한 상태와 색인을 조회할 수 없는 상태도 나눴습니다. 요약 기능 역시 비용 제한, 생성 장애, 원문 부재를 각각 별도의 상태로 다룹니다.',
        ),
        paragraph(
          'project-docsuri-consumers-cache-personalization',
          '캐시 재사용 조건과 개인화 가중치 제한\n파생 캐시는 논문, 버전, 언어, persona, 용어집, 모델, 프롬프트, DocModel 세대를 구분합니다. 개인화 가중치는 개별 값뿐 아니라 절댓값의 합에도 상한을 뒀습니다. 세 범주의 점수가 같아지는 반례는 속성 테스트의 고정 사례로 추가했습니다.',
        ),
        paragraph(
          'project-docsuri-consumers-evaluation-citations',
          '요약 근거 평가와 인용 그래프의 한계 표시\n요약의 근거를 평가할 때는 실제 논문 8건에서 발췌한 수치로 18개 사례를 만들고, 잘못 통과시킨 경우(false-pass)와 불필요하게 판단을 보류한 경우(false-abstain)를 구분했습니다. confident로 분류된 14개 사례의 성공 기록은 이 통제된 평가 범위에만 해당합니다. 인용 그래프는 대표 식별자를 찾지 못한 경우, 순환·중복, 탐색량 제한, 부분 결과를 표현하도록 설계했습니다.',
        ),
      ],
      outcomes: [
        paragraph(
          'project-docsuri-outcomes-0-text',
          '장애를 재현한 질의로 검색 복구 확인\n7월 1일 워커를 3개에서 1개로 줄인 뒤, 장애를 재현했던 질의가 정상 응답하는 것을 확인했습니다. 다음 날 추가로 안정화하면서는 큐를 보존한 채 수집과 자동 확장을 일시 정지했고, 검색은 기능 저하 상태가 아닌 정상 결과 20건을 반환했습니다. 아래 수치는 당시 문서와 커밋에 남아 있는 관측값입니다.',
        ),
        paragraph(
          'project-docsuri-outcomes-bounded-canary',
          '논문 한 편을 대상으로 재처리 검증\n단일 논문 파일럿에서 118개 청크와 13개 자산을 생성했습니다. 처리 대상 큐의 잔량은 0, 수집 DLQ는 111건으로 유지됐고, 검색에도 저하 표시가 없었습니다. 7월 8일 문서 생성 DLQ가 24건에서 0건이 된 것은 다른 큐의 후속 복구 기록입니다. 이 결과가 전체 백필의 완료나 장기적인 SLA 충족을 뜻하지는 않습니다.',
        ),
        paragraph(
          'project-docsuri-outcomes-mixed-load',
          '혼합 부하에서 측정한 응답 시간과 오류율\n조치 후 가상 사용자 20명(20 VU)으로 실행한 k6 기록에서 검색 p95는 664.9 ms, 전체 HTTP 오류율은 0.01%였습니다. 프런트엔드, readyz, 검색 요청을 섞은 테스트이며 하나의 질의를 반복했으므로 캐시의 영향이 있을 수 있습니다. 이전에 측정한 검색 p95 9,061 ms는 요청 12회·동시성 3이라는 다른 조건에서 나온 값이므로, 두 결과로 개선율을 계산하지 않았습니다.',
        ),
        paragraph(
          'project-docsuri-outcomes-runtime-diagnosis',
          '실제 운영 설정과 코드의 차이 확인\n별도의 작업 적체 조사에서는 약 15.3만 건이 대기하는 상황에서 코드와 템플릿의 확장 상한은 1인데 실제 설정은 0이라는 차이를 확인했습니다. 워커가 실행되지 않던 원인을 좁혀 간 기록입니다. 관측 모듈에는 중복 이벤트를 추적하는 상태의 메모리 상한과 로그의 민감 정보 가림 처리를 보완했습니다. 다만 오래된 ID를 제거한 뒤에는 같은 이벤트를 다시 받을 가능성이 남습니다.',
        ),
        paragraph(
          'project-docsuri-outcomes-recovery-checks',
          '환경별로 확인한 백업과 복구 범위\nAWS 운영 당시에는 팀이 CloudWatch 이메일 수신을 확인하고, RDS snapshot을 새 인스턴스로 복원해 available 상태가 되는 것을 확인했습니다. 로컬 환경으로 옮긴 뒤에는 launchd로 백업을 실행하고 로컬과 iCloud 보관 경로에 dump 파일이 생성된 것을 확인했습니다. dump 생성 확인은 복원 시험과 다르며, AWS에서 수행한 검증도 현재 객체 저장소와 검색 인덱스를 함께 복원할 수 있다는 보장은 아닙니다.',
        ),
      ],
      lessons: [
        paragraph(
        ...
        paragraph(
          ...
          '더 큰 데이터 플랫폼으로 확장하고 싶습니다\n여러 출처의 데이터를 정규화하고 결합한 경험, 다시 시작할 수 있는 배치를 만든 경험, 읽기·쓰기 서비스의 데이터 규칙과 부하·비용·복구를 다룬 경험을 더 큰 데이터 플랫폼에서 이어 가고 싶습니다. 새로운 기술을 선택할 때도 데이터를 만드는 과정과 이를 사용하는 동료의 요구를 함께 살피겠습니다.',
        ),
      ],
      evidence: [
        {
    ...
    },
    {
      ...
      outcomeSummary: fact(
        ...
        '로컬 Zotero 라이브러리를 의미 기반으로 검색하는 MCP 서버를 만들었습니다. 임베딩 벡터 색인과 PDF 본문·도판 추출, Harvard 형식 인용 생성을 제공합니다.',
      ),
      problem: [
        paragraph(
      ...
      role: [
        paragraph(
          ...
          '저장소 소유자로서 MCP 도구 설계부터 로컬 데이터 접근, 의미 기반 검색 색인, 인용 출력까지 전체 흐름을 구현했습니다. Python, SQLite, PyMuPDF, ChromaDB를 사용했습니다.',
        ),
      ],
      keyDecisions: [
        paragraph(
          'project-mcp-local-reference-key-decisions-0-text',
          'Zotero의 SQLite 데이터베이스와 로컬 파일을 읽어 메타데이터 검색과 의미 기반 검색 인덱스를 결합했습니다. 원본 데이터와 검색 색인은 분리하고, 자료 조회·도판 추출·인용 생성은 각각 별도의 도구로 제공했습니다.',
        ),
      ],
      architecture: [
        paragraph(
          'project-mcp-local-reference-architecture-0-text',
          'Python MCP 서버가 Zotero 메타데이터와 컬렉션, PDF 본문·도판, 의미 기반 검색 인덱스를 연결하고 클라이언트의 요청에 결과를 반환합니다. 벡터 색인은 원본 데이터와 분리된 경로에 두고, 라이브러리가 바뀌면 다시 만들 수 있도록 구성했습니다.',
        ),
      ],
      outcomes: [
        paragraph(
          'project-mcp-local-reference-outcomes-0-text',
          '하나의 로컬 서버에서 서지 메타데이터 검색, 컬렉션 탐색, PDF 본문·도판 추출, Harvard 형식 인용 생성, 라이브러리 색인을 제공합니다. 의미 기반 검색은 질의와 같은 단어가 제목에 없어도 관련 자료를 찾습니다.',
        ),
      ],
      lessons: [
        paragraph(
    ...
    },
    {
      ...
      outcomeSummary: fact(
        ...
        'Obsidian 문서를 Rust 전처리와 Astro로 정적 사이트에 게시하는 파이프라인을 만들었습니다. 렌더러 실패가 CI에 전달되지 않던 문제를 고쳐, 실패가 배포 단계에서 감지되도록 했습니다.',
      ),
      problem: [
        paragraph(
          'project-obsidian-custom-publish-problem-0-text',
          'Obsidian 문서의 위키링크, 검색, 탐색 트리, 미리보기, 다이어그램을 정적 사이트에서 사용할 수 있게 만드는 개인 프로젝트입니다. 문서를 읽어 콘텐츠와 검색·그래프 데이터를 만드는 전처리, HTML 변환, 배포를 하나의 재현 가능한 흐름으로 이어야 했습니다.',
        ),
        paragraph(
          'project-obsidian-custom-publish-problem-renderer-exit',
          '렌더링 실패가 CI에 전달되지 않은 원인\n2026년 7월에는 CI가 성공으로 끝났지만 28개 글에서 Mermaid 렌더링 65건이 실패해 다이어그램이 빠진 문서가 배포됐습니다. 로컬 브라우저 설정 파일이 Git 관리 대상에서 제외돼 CI 환경으로 전달되지 않았고, 렌더러 오류가 경고로만 처리되면서 전처리 명령도 성공으로 끝났습니다.',
        ),
      ],
      role: [
        paragraph(
      ...
      keyDecisions: [
        paragraph(
          ...
          '문서 스캔, 링크 처리, 변환, 검색 데이터 생성, 출력으로 이어지는 전처리를 Astro 렌더링과 분리했습니다. CI에는 브라우저 실행 경로를 명시하고, Mermaid·D2의 SVG 렌더링이 실패하면 이를 집계해 전처리 명령도 실패로 종료하도록 바꿨습니다.',
        ),
      ],
      architecture: [
        paragraph(
    ...
    },
    {
      ...
      outcomeSummary: fact(
        ...
        '음성·이미지·텍스트 입력을 Gemini에 전달하는 Flutter 앱과, API 키를 보호하는 Cloud Run 중계 계층을 구현했습니다.',
      ),
      problem: [
        paragraph(
