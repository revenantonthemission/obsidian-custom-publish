...
- [x] 모든 `[Answer]:`를 수집하고 형식·명확성·상호 일관성·기존 승인 호환성을 검증한다.
- [x] 모호한 답변이 있으면 clarification file로 해소한다. 해당 없음 — 1차 제출의 한 칸 밀림(Q3 공란, Q3-A 본문이 Q4 태그에 위치)은 내용 의미가 유일해 기계적으로 재배치했고, Q4는 추정 없이 재질문해 사용자가 직접 A를 기입했다. 최종 10개 답변이 모두 명확한 A 선택이다.

### 4.3 생성과 검증

- [x] 측정 가능한 U2 NFR catalog(`nfr-requirements.md`)를 생성한다 (NFR-U2-001~011).
- [x] 도구 결정 기록(`tech-stack-decisions.md`)을 생성한다 (TSD-U2-01~07).
- [x] 선택된 Rust PBT framework를 dev-dependency로 설치하고 PBT-09 smoke(custom generator, shrinking, seed 재현, runner 통합)를 증명한다 — proptest 1.11.0, 87 test 통과.
- [x] 독립 검토와 Markdown/링크 구조 검증을 수행한다 — BLOCKING 0, MATERIAL 2, MINOR 3 모두 수정 반영.
- [x] 표준 2-option 완료 gate를 제시하고 명시적 승인을 기다린다.

## 8. Artifact Validation Result

- **검증 완료 시각**: 2026-07-28
- **필수 artifact**: 2/2 생성 — `nfr-requirements.md`(NFR-U2-001~011)와 `tech-stack-decisions.md`(TSD-U2-01~07), `aidlc-docs/construction/homepage-publication-boundary/nfr-requirements/`.
- **PBT-09**: Compliant — 실행 증명 기반. proptest 1.11.0 dev-dependency 설치; 한국어/Unicode 도메인 구조적 Strategy 2개 property × 256 cases 통과; 일회용 실패 property가 최소 반례 `[0,0,0,0,0]`으로 shrink; 지속 파일 `cc d5a76a82…` 기록 후 재실행이 0.00s·successes 0으로 정확 재현; 증거 확보 후 일회용 property와 지속 파일 제거. 전체 `cargo test` 87 passed / 0 failed, 약 26초 (3분 예산 내 baseline).
...
[Answer]: A) `proptest`를 dev-dependency로 채택한다. `Strategy` 조합으로 구조적 custom generator를 만들고, 통합 shrinking, `proptest-regressions/` 실패 지속, 환경 변수 기반 seed 재현과 `cargo test` 통합을 그대로 사용한다.
...
[Answer]: A) framework의 실패 지속 파일(`proptest-regressions/*.txt` 등)을 저장소에 커밋해 모든 과거 실패가 매 실행마다 재검사되게 하고, 의미 있는 최소 반례는 FR-015.6에 따라 별도의 명명된 example test로도 승격한다. 실패 출력의 seed로 같은 실패를 재현할 수 있어야 한다.
...
[Answer]: A) property당 framework 기본값(proptest 기준 256 cases)을 사용하고, `just test` 전체(기존 85 test + U2 추가)는 로컬 기준 3분 이내를 유지한다. 예산 초과 시 개별 property의 케이스 수를 명시적으로 조정하고 그 근거를 기록한다.
...
[Answer]: A) 같은 입력(vault 내용 + 코드)에서 연속 두 번 실행한 `content/` 전체 tree(파일 목록·내용)가 byte 동일해야 한다. timestamp, 실행 환경, hash map 순회 순서 같은 비결정 원천을 산출물에서 배제하고, fixture 기반 자동 test로 검증한다.
...
[Answer]: A) stderr에 진단당 한 줄, `PUB### {path}: {detail}` 형식으로 정렬 순서 그대로 출력하고 종료 코드는 1이다. 기계 파싱용 JSON 출력은 도입하지 않는다 — 소비자는 사람과 CI 로그뿐이다.
...
[Answer]: A) "오늘" 값을 build 시작 시 한 번 계산해 조합 로직에 주입 가능한 매개변수로 만든다. production build는 실제 날짜를 쓰고, test는 고정 날짜를 주입해 글 있음/없음 두 경우를 결정적으로 검증한다.
...
[Answer]: A) 자동 검증만 적용한다: U1과 같은 axe 규칙(WCAG 2.0/2.1 A/AA + 2.2 AA)을 `/`에 실행하고 keyboard·reduced-motion Playwright smoke를 포함한다. U1의 수동 review matrix는 U1 profile 계약 소유이므로 확장하지 않는다. `/`는 기존 site shell을 재사용하고 새 상호작용 요소가 없다.
...
[Answer]: A) U1과 동일하게 chromium·firefox·webkit × 320×800·1440×900을 재사용한다. 도구 버전도 U1이 pin한 Playwright 구성을 따른다.
...
[Answer]: A) 새 client JavaScript 0 bytes(기존 island·script 유지), 새 외부 origin 요청 0. slot fragment 스타일은 기존 global/profile 스타일 재사용을 우선하고, 신규 CSS를 추가하는 경우 gzip 4KiB 이내로 제한한다.
...
[Answer]: A) 신규 명령을 만들지 않는다. Rust는 `just test`(= `cargo test`, U2 example·PBT 포함, 실패 시 seed 확인 가능)가 U2의 stable command이고, site는 U1의 기존 `npm run test:unit`·`test:pbt`·`test:e2e`에 U2 suite를 추가한다. U3는 이 기존 명령들을 변경 없이 aggregation한다.

## 6. Answer Validation Result

- **1차 제출**: 2026-07-28, 사용자 응답 "작성 완료" — 9/10. Q3 태그가 비어 있고 Q3-A 본문이 Q4 태그에 있었다. Q3-A 본문은 의미가 유일해 U1 선례에 따라 Q3로 기계적 재배치했고, Q4는 A/B가 실질적으로 다른 약속이므로 추정하지 않고 gate를 차단한 채 재질문했다.
- **2차 제출**: 2026-07-28, 사용자 응답 "작성 완료" — Q4에 A 본문이 직접 기입되어 10/10 완결.
- **답변**: A/A/A/A/A/A/A/A/A/A
- **형식·명확성 검증**: 통과. 모든 답변이 해당 질문의 A 선택지 본문과 정확히 일치하며 조건부·혼합·`X` 답변이 없다.
- **상호 일관성 검증**: 통과. proptest 채택(Q1)이 regressions 커밋·example 승격(Q2)과 기본 256 cases(Q3)의 전제를 제공하고, byte 결정성(Q4)이 진단 정렬(Q5)·시간 주입(Q6)과 하나의 결정적 검증 체계로 연결된다. 자동 접근성(Q7)·U1 matrix 재사용(Q8)·자원 상한(Q9)·기존 명령 재사용(Q10)이 U3 aggregation 경계와 상충 없이 맞물린다.
- **기존 승인 호환성**: 통과. FD 승인 결정, TS 도구 재사용, Cargo.toml 변경 한정, no-JS·no-deploy 경계를 어느 답변도 변경하지 않는다.
- **Clarification Status**: Not required (재질문은 clarification file이 아니라 미완결 답변의 완성이다).

| Question | Answer | Confirmed NFR Decision |
|---|---|---|
| Q1 | A | proptest dev-dependency; Strategy 기반 custom generator, 통합 shrinking, 실패 지속, cargo test 통합 |
| Q2 | A | `proptest-regressions/` 커밋 + 의미 있는 최소 반례의 명명된 example 승격 + seed 재현 |
| Q3 | A | 기본 256 cases/property; `just test` 로컬 3분 예산; 조정은 명시·기록 |
| Q4 | A | 연속 두 실행의 `content/` tree byte 동일; 비결정 원천 산출물 배제; fixture 자동 test |
| Q5 | A | stderr `PUB### {path}: {detail}` 한 줄/진단, 정렬 순서, exit 1, JSON 없음 |
| Q6 | A | "오늘" 값 build 시작 1회 계산·주입 가능; test는 고정 날짜로 양 경우 검증 |
| Q7 | A | `/`는 자동 axe(WCAG 2.0/2.1 A/AA + 2.2 AA)·keyboard·reduced-motion smoke만; 수동 matrix 미확장 |
| Q8 | A | chromium·firefox·webkit × 320×800·1440×900, U1 pin Playwright 재사용 |
| Q9 | A | 새 client JS 0 bytes, 새 외부 origin 요청 0, 신규 CSS gzip 4KiB 이내 |
| Q10 | A | 신규 명령 없음; `just test`가 Rust stable command, site는 기존 npm script에 suite 추가 |
